...
type PlacementBase = {
  ...
  question: string;
  /**
   * 지문(영어 문장)의 한국어 해석. 빈칸은 정답을 채운 완성 문장으로 옮긴다.
   *
   * 문제를 푸는 동안에는 절대 보여 주지 않는다. "카드 단말기가 안 돼요.
   * 현금 있으세요?" 를 띄우면 답이 cash 라고 알려 주는 셈이다. 답을 낸 뒤
   * 채점 화면과 오답 설명지에서만 영어 문장 밑에 붙인다.
   */
  ko?: string;
  /**
   * 정답 자체가 문장인 문항(주로 활용)의 해석. 지문이 따로 없으므로
   * 정답 옆에 붙인다.
   */
  answerKo?: string;
