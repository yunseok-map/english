...
import { Check, ChevronRight, RefreshCw, Settings2, Trash2, Volume2 } from "lucide-react";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import { Panel, Eyebrow, Empty } from "@/components/Panel";
import { useApp } from "@/state/context";
import { LEVEL_DESC, LEVEL_LABEL, LEVEL_ORDER, nextLevel, promotionProgress } from "@/lib/level";
import { daysTo } from "@/lib/engine";
import { speak } from "@/lib/speech";
import type { Level } from "@/types";

export function ProfileScreen({ startRetest, openSettings }: { startRetest: () => void; openSettings: () => void }) {
  ...
  const promote = () => {
    ...
    update((state) => ({
      ...
      profile: {
        ...state.profile,
        level: upcoming,
        levelHistory: [...state.profile.levelHistory, { level: upcoming, at: new Date().toISOString(), source: "promotion" }],
      },
    }));
  ...
  const avgPronunciation = app.stats.pronunciationScores.length
    ? Math.round(app.stats.pronunciationScores.reduce((a, b) => a + b, 0) / app.stats.pronunciationScores.length)
    : 0;
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <h1 className="mt-1 text-[1.375rem] font-bold tracking-tight">{app.profile.name}님의 훈련 기록</h1>
        <p className="mt-1 text-[0.875rem] text-muted-foreground">
          출국까지 {daysTo(app.profile.departureDate)}일 · 연속 학습 {app.stats.streak}일
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {[
          ...
          <Panel key={String(label)} className="py-4 text-center">
            <b className="block font-mono text-[1.5rem] font-semibold tabular-nums">{value}</b>
      ...
      <Panel className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={startRetest} className="inline-flex min-h-9 items-center gap-1 text-[0.8125rem] font-semibold text-primary">
        ...
        <div className="space-y-2.5">
          {LEVEL_ORDER.map((item: Level) => {
            ...
            return (
              <div key={item} className={`flex gap-3 rounded-xl p-3 ${current ? "bg-accent/50" : ""}`}>
                <span
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[0.9375rem] font-semibold">
                    ...
                    {current && <span className="ml-1.5 text-[0.75rem] font-medium text-primary">현재</span>}
                  </p>
                  <p className="text-[0.8125rem] leading-relaxed text-muted-foreground">{LEVEL_DESC[item]}</p>
                </div>
              </div>
            );
          })}
        ...
        {upcoming && (
          <div className="space-y-2.5 rounded-xl border p-3">
            ...
            <div className="space-y-2">
              <div>
                ...
                <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-muted">
                  <i className="block h-full rounded-full bg-primary" style={{ width: `${Math.min(100, progress.wordMastery * 100)}%` }} />
                </div>
              </div>
              <div>
                <div className="flex justify-between text-[0.75rem] text-muted-foreground">
                ...
                <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-muted">
                  <i
                    ...
                    style={{ width: `${progress.lessonsTotal ? (progress.lessonsDone / progress.lessonsTotal) * 100 : 0}%` }}
                  />
                </div>
              </div>
            </div>
            {progress.eligible ? (
              ...
              <p className="text-[0.75rem] text-muted-foreground">두 조건을 모두 채우면 승급할 수 있어요. 재테스트로 바로 확인할 수도 있어요.</p>
            )}
          </div>
        )}
        ...
        {app.profile.levelHistory.length > 0 && (
          <div className="space-y-1.5">
            ...
            {[...app.profile.levelHistory].reverse().slice(0, 5).map((event, i) => (
              <p key={i} className="flex justify-between text-[0.8125rem] text-muted-foreground">
                <span>
                  {event.level} {LEVEL_LABEL[event.level]} ·{" "}
                  {event.source === "placement" ? "레벨 테스트" : event.source === "retest" ? "재테스트" : event.source === "promotion" ? "승급" : "이전 기록"}
                </span>
      ...
      <section className="space-y-2.5">
        <Eyebrow className="text-muted-foreground">내 문장장 {app.savedPhrases.length}</Eyebrow>
        {app.savedPhrases.length === 0 ? (
          <Empty title="아직 저장한 문장이 없어요" text="변환기나 회화팩에서 마음에 드는 문장을 저장해 보세요." />
        ) : (
          <div className="overflow-hidden rounded-2xl border bg-card">
            {app.savedPhrases.slice(0, 20).map((phrase, i) => (
              <div key={phrase.id} className={`flex items-center gap-2 px-4 py-3 ${i > 0 ? "border-t" : ""}`}>
                <span className="min-w-0 flex-1">
                  <span className="block font-mono text-[0.875rem] [overflow-wrap:anywhere]">{phrase.en}</span>
                  <span className="block truncate text-[0.75rem] text-muted-foreground">{phrase.ko}</span>
                </span>
                <button
                ...
                <button
                  onClick={() => update((state) => ({ ...state, savedPhrases: state.savedPhrases.filter((p) => p.id !== phrase.id) }))}
                  aria-label="삭제"
                  className="flex size-10 shrink-0 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted"
                >
                  <Trash2 size={15} />
